...
def data_process(data_path, name, patch_size, stride, mode):
    ...
    if name == "DRIVE":
        ...
        gt_path = os.path.join(data_path, mode, "mask")
    ...
    for i, file in enumerate(file_list):
        if name == "DRIVE":
            ...
            file_number_str = file.split('.')[0]
            gt_filename = f"{int(file_number_str):02d}_test_mask.gif"
            gt = Image.open(os.path.join(gt_path, gt_filename))
            img = Grayscale(1)(img)
